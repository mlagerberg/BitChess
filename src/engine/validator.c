...
static int get_valid_moves_pawn(Move **head, Board *board, int x, int y, int color);

static int get_valid_moves_knight(Move **head, Board *board, int x, int y, int color);

static int get_valid_moves_rook(Move **head, Board *board, int x, int y, int color);

static int get_valid_moves_bishop(Move **head, Board *board, int x, int y, int color);

static int get_valid_moves_queen(Move **head, Board *board, int x, int y, int color);

static int get_valid_moves_king(Move **head, Board *board, int x, int y, int color);
...
static int get_all_valid_moves_of_piece(Move **head, Board *board, int i, int j, bool only_count) {
	...
	if (piece->shape == PAWN) {
		count = get_valid_moves_pawn(head, board, i, j, piece->color);
	} else if (piece->shape == KNIGHT) {
		count = get_valid_moves_knight(head, board, i, j, piece->color);
	} else if (piece->shape == BISHOP) {
		count = get_valid_moves_bishop(head, board, i, j, piece->color);
	} else if (piece->shape == ROOK) {
		count = get_valid_moves_rook(head, board, i, j, piece->color);
	} else if (piece->shape == QUEEN) {
		count = get_valid_moves_queen(head, board, i, j, piece->color);
...
static int get_valid_moves_pawn(Move **head, Board *board, int x, int y, int color) {
	...
	if (Board_get_piece(board, x, y - color) == NULL) {
		add_move_pawn(head, color, x, y, x, y - color);
		count++;
		if (y == startY && Board_get_piece(board, x, y - 2*color) == NULL) {
			add_move(head, Move_create(color, x, y, x, y - 2*color, 0));
			count++;
		}
	}
	xx = x-1; yy = y - color;
	if (x > 0 && Board_is_color(board, xx, yy, -color)) {
		add_move_pawn(head, color, x, y, xx, yy);
		count++;
	}
	xx = x+1; yy = y - color;
...
static int get_valid_moves_rook(Move **head, Board *board, int x, int y, int color) {
	...
	while (i >= 0 && (Board_get_piece(board, i, y) == NULL || Board_get_piece(board, i, y)->color != color)) {
		add_move(head, Move_create(color, x, y, i, y, 0));
		count++;
	...
	while (j >= 0 && (Board_get_piece(board, x, j) == NULL || Board_get_piece(board, x, j)->color != color)) {
		add_move(head, Move_create(color, x, y, x, j, 0));
		count++;
...
static int get_valid_moves_bishop(Move **head, Board *board, int x, int y, int color) {
	...
	while (i >= 0 && j >= 0 && (Board_get_piece(board, i, j) == NULL || Board_get_piece(board, i, j)->color != color)) {
		add_move(head, Move_create(color, x, y, i, j, 0));
		count++;
...
static int get_valid_moves_queen(Move **head, Board *board, int x, int y, int color) {
	int count;
	count = get_valid_moves_rook(head, board, x, y, color);
	count += get_valid_moves_bishop(head, board, x, y, color);
	return count;
}
...
static int get_valid_moves_king(Move **head, Board *board, int x, int y, int color) {
	...
	if (color == BLACK && y == 0 && x == 4 && !v_square_gives_check(board, 4, 0, color)) {
		if (board->black_can_castle_queens_side
				...
				add_move(head, Move_create(color, x, y, x-2, y, 0));
			count++;
		}
